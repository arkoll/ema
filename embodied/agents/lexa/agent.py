...
class Agent(tfagent.TFAgent):
    ...
    def __init__(self, obs_space, act_space, step, config):
        ...
        self.achiever = getattr(behaviors, config.achiever_behavior)(
            self.wm, self.act_space, self.config
        )
        self.explorer = getattr(behaviors, config.explorer_behavior)(
            self.wm, self.act_space, self.config
        )
    ...
    def policy(self, obs, state=None, mode='train'):
        ...
        outs = {
            **outs, 'action': tfutils.action_noise(
    ...
    def train(self, data, state=None):
        ...
        _, mets = self.achiever.train(self.wm.imagine, start, context)
        metrics.update({
            'ach_' + key: value for key, value in mets.items()
        })
        _, mets = self.explorer.train(self.wm.imagine, start, context)
        metrics.update({
